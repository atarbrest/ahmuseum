...
class DetailsBloc extends BaseBloc<DetailsEvent, DetailsState> {
  ...
  FutureOr<void> _onFetchDetails(FetchDetails event, Emitter<DetailsState> emit) async {
    emit(Loading());

    await makeExecution<ArtObjectDetails>(
      function: () async {
        final artDetails = await detailsRepository.fetchArtDetails(objectNumber: event.objectNumber);
        return artDetails;
      },
